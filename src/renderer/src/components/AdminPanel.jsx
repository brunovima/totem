...
export default function AdminPanel({ onLogout }) {
  ...
  // Injeta focus ring visível em todos os inputs do painel
  useEffect(() => {
    const style = document.createElement('style')
    style.id = 'totem-admin-focus'
    style.textContent = [
    ...
    if (!document.getElementById('totem-admin-focus')) document.head.appendChild(style)
    return () => document.getElementById('totem-admin-focus')?.remove()
  }, [])
  ...
  const handleCreateQuiz = async () => {
    ...
    try {
      await window.api.createQuiz(title)
      setQuizTitle('')
      showFeedback(`Quiz "${title}" criado com sucesso!`)
      refreshData()
    } catch {
      showFeedback('Erro ao criar quiz. Tente novamente.')
    }
  }

  const handleSaveQuestion = async () => {
    const filteredOptions = options.filter((o) => o.trim() !== '')
    if (!qText.trim()) { showFeedback('Digite o texto da pergunta.'); return }
    if (filteredOptions.length < 2) { showFeedback('Adicione pelo menos 2 alternativas.'); return }
    if (parseInt(correct) >= filteredOptions.length) { showFeedback('Marque a alternativa correta.'); return }
    try {
    ...
    <div style={{ display: 'flex', width: '100vw', height: '100vh', backgroundColor: '#f1f5f9', color: '#0f172a', fontFamily: "'Roboto', sans-serif" }}>
      ...
      <aside style={{ width: '220px', background: '#0f172a', color: 'white', padding: '20px', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        ...
        <button
          ...
          style={{
            padding: '12px',
            background: kioskActive ? '#7c3aed' : '#16a34a',
            color: 'white', border: 'none', borderRadius: '8px',
            cursor: 'pointer', fontWeight: 'bold', fontFamily: FONT,
                  ...
                  {leads.map((l) => (
                    <tr key={l.id} onClick={() => toggleSelectLead(l.id)} style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer', background: selectedLeads.has(l.id) ? '#eff6ff' : 'white' }}>
                      ...
                      <td style={{ padding: '14px', textAlign: 'center' }}>
                        {l.autorizado ? (
                          <span style={{ background: '#dcfce7', color: '#16a34a', padding: '2px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 700 }}>✓ Sim</span>
                        ) : (
